...
contract Voting is OwnableUpgradeable, IVoting {
  ...
  uint256 private constant minThresholdPercentage = 30; // 3%
  uint256 private constant maxThresholdPercentage = 500; // 50%

  uint256 public voteCount; //Number of votes in contract
  ...
  function initalize(address _governToken) public initializer {
  ...
  function updateThreshold(uint256 _threshold) external onlyManagers {
    require(minThresholdPercentage <= _threshold && _threshold <= maxThresholdPercentage, "Out of Range");
  ...
  function viewVote(uint256 id) public view virtual override returns (Vote memory) {
    require(id <= voteCount - 1, "Not exist");

    return votes[id];
  }

  function viewNumberOfVoteMake(address user) public view virtual override returns (uint256) {
    return userProposal[user].length;
  }

  function viewUserVote(uint256 _id, address user) public view virtual override returns (bool) {
    return userVote[user][_id];
  }

  function viewCurrentVoteId() public view virtual override returns (uint256) {
    return voteCount - 1;
  }

  function viewYourVote(address user) public view virtual override returns (uint256[] memory) {
